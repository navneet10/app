...
function ServiceSection() {
    ...
    useEffect(() => {
        const titledata = async () => {
            try {
                const titledatalist = await fetchData("all_title");
                const filtered = titledatalist.find(item => item.id === 3);
                setTitle(filtered);
                console.log("Fetched titles:", filtered); // Safe to inspect
            } catch (error) {
    ...
    return (
        <> 
            <section className="position-relative overflow-hidden box-section box-our-services-11">
                <div className="container">
                    <div className="row position-relative align-items-end">
                        <div className="col-lg-8">
                            <p className="sub-heading-ag-sm text-uppercase primary-500 mb-1">
                                {title?.small_text}
